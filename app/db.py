...
def connect(db_path: str) -> sqlite3.Connection:
    ...
    # Better concurrency and faster writes for long-running evaluation sessions.
    # (WAL works best when DB is on persistent disk.)
    try:
...
def append_message(conn: sqlite3.Connection, session_id: str, sender: str, text: str, timestamp_ms: int) -> None:
    conn.execute(
        """
        INSERT INTO messages (session_id, sender, text, timestamp_ms)
        VALUES (?, ?, ?, ?)
        """,
        (session_id, sender, text, int(timestamp_ms)),
    )
    conn.execute(
...
def update_session(
    ...
    conn.execute(
        ...
        """,
        (
            int(bool(scam_detected)),
            float(confidence),
            last_reply,
            int(bool(engagement_complete)),
            agent_notes,
            int(bool(callback_pending)),
...
def list_messages(conn: sqlite3.Connection, session_id: str, limit: int = 30) -> list[dict[str, Any]]:
    ...
    return [{"sender": row["sender"], "text": row["text"], "timestamp": row["timestamp_ms"]} for row in rows]
